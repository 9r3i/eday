...
foreach($posts as $post){
  ...
  echo '<div class="post-row">'
    ...
    .'<a href="'.site::url.'?'.EDAY_ADMIN_KEY.'=post/edit/'.$post['id'].'" class="submit-green">'
...
if($next){
  echo '<div class="post-row" style="text-align:center;padding:20px;">'
    .'<a href="'.site::url.'?'.EDAY_ADMIN_KEY.'=post/all/'.$next.'/'.$limit
      .'" class="submit submit-blue">Next Posts</a>'
    .'</div>';
}
?>
</div>
<script type="text/javascript" src="<?=admin::appURL('js/post.js')?>"></script>
<script type="text/javascript">
var POST_ACTION_URL=SITE_URL+'?'+SITE_ADMIN_KEY+'=post/ajax';
</script>
<?php
endOfPostAll:
