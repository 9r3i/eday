<div class="index" style="margin:20px auto;max-width:500px;">
<?php
/* use namespace eday */
use eday\admin;
global $token;
...
if(save_button){save_button.addEventListener('click',function(e){
  ...
  W.post(SITE_URL+'?'+SITE_ADMIN_KEY+'=log/save/setup',function(r){
    ...
    if(r.toString().match(/^error/ig)){
      ...
      return success('Saved.',function(y){
        W.location.assign(SITE_URL+'?'+SITE_ADMIN_KEY+'=log/in');
      });
    }console.log(r);
    save_button.disabled=false;
...
<script type="text/javascript" src="<?=admin::appURL('js/footer.js')?>" async="true"></script>
</body></html>
