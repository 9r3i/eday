...
echo '<div class="post-row">Total users: '.$row
  .'<a href="'.site::url.'?'.EDAY_ADMIN_KEY.'=user/add" class="submit-green menu-add-button" title="Add User">'
...
foreach($users as $user){
  ...
  if(EDAY_ADMIN_TYPE=='admin'||EDAY_ADMIN_TYPE=='master'){
    ...
    if(EDAY_ADMIN_TYPE=='master'||EDAY_ADMIN_ID==$user['id']){
      $option.='<a href="'.site::url.'?'.EDAY_ADMIN_KEY.'=user/edit/'.$user['id'].'" class="submit-green">'
...
if($next){
  echo '<div class="post-row" style="text-align:center;padding:20px;">'
    .'<a href="'.site::url.'?'.EDAY_ADMIN_KEY.'=user/all/'.$next.'/'.$limit.'" class="submit submit-blue">Next Users</a>'
    .'</div>';
}
?>
</div>
